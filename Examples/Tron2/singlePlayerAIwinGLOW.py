...
Playerpoints = 0
Totalpoints = 0
screen = pygame.display.set_mode((WINDOWW, WINDOWH), pygame.DOUBLEBUF | pygame.FULLSCREEN )
TRAIL_LIMIT = 90
TCE = TRAIL_LIMIT
CLOCKRATE = 30
...
class LightBike(pygame.sprite.Sprite):
    def __init__(self, controls = 3, SpSet = "lights/GLOWGREEN"):
        ...
        self.image.blit(pygame.image.load(self.sprite_reference),(0,0))
...
# Actually sets the sprite, using the reference
        ...
        if len(self.prev_pos) > (TRAIL_LIMIT + self.TB + (lb.level - 1)*20):
            self.prev_pos.pop(0)
            if len(self.prev_pos) > (TRAIL_LIMIT + self.TB + (lb.level - 1)*20):
                self.prev_pos.pop(0)
...
# Actually sets the sprite, using the reference
        ...
        if len(self.prev_pos) > (TRAIL_LIMIT + lb.TB + (lb.level - 1)*20):
            self.prev_pos.pop(0)
            if len(self.prev_pos) > (TRAIL_LIMIT + lb.TB + (lb.level - 1)*20):
                self.prev_pos.pop(0)
...
lb2 = LightBike4( "R", 14, 14, 14, "lights/GLOWBLUE")
lb3 = []
lb3.append(LightBike4("L", 9, 9, 9))
lb3.append(LightBike4("R", 9, 9, 9))
lb3.append(LightBike4("U", 9, 9, 9))
lb3.append(LightBike4("D", 9, 9, 9))
lb3[0].thisimage = "tanks/KILLER"
lb3[1].thisimage = "tanks/DO"
lb3[2].thisimage = "tanks/DO"
lb3[3].thisimage = "tanks/DO"
...
def main():
    ...
    while play:
        ...
        TITLE = Title.render( ("HP: "+ str(lb.HP)), False, ((COL/5) % 254, (COL/ 7) % 254, (COL/9) % 254))
        ...
        #screen.blit(bgi,(0,0))
        screen.fill((255-(lb.TB*1.2),255-(lb.TB*1.2),255-(lb.TB*1.2)))
