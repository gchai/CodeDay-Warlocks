import sys, random
import pygame
import GenerateBGI
from pygame.locals import *
sys.path.append( 'Health/' )
import health
sys.path.append( 'ColorMod/' )
import ColorMod
pygame.init()

Winw = 5.05
Winh = 3
SPEED = 12
WINDOWW=  int(16 * SPEED * Winw)
WINDOWH =  int(18 * SPEED * Winh)
IQ = 10#1-10 value
Playerpoints = 0
Totalpoints = 0
screen = pygame.display.set_mode((WINDOWW, WINDOWH))
TRAIL_LIMIT = 90
TCE = TRAIL_LIMIT
COL = pygame.time.get_ticks()
mod = 5
# make each direction an integer. put transparent images as element 0 in the list. get rid of self.sprite_reference
#
#
#
#
#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~testing ColorModGui~~~~~~~~~~~
path = 'ColorMod/'
sys.path.append( path )
sys.path.append( 'Animation/Foxreflector/' )
import ClassReflector
class LightBike(pygame.sprite.Sprite):
    def __init__(self, controls = 3, SpSet = "lights/GLOWGREEN"):
        import ColorModGui
        _content = ColorModGui.GetIMG(screen,"lights/", ["B1","B2","B3"],"LightBike 1 Mod")
        _content[0].blit(_content[1],(0,0))
        _content[0].blit(_content[2],(0,0))
        spr = _content[0]
        self.Color = _content[3]
        self.reflect = ClassReflector.Reflector(screen,True, self.Color,[48,48])
        self.reflect2 = ClassReflector.Reflector(screen,True, self.Color,[12,12])
        self.refVel = [0,0]
        self.refVel2 = [0,0]
        #TODO self.FR = ClassReflector.Reflector(Screen, True, "b", (24,24), "Animation/Foxreflector/")
        if self.Color[0] < 25:
            self.Color[0] = 26
        if self.Color[1] < 25:
            self.Color[1] = 26
        if self.Color[2] < 25:
            self.Color[2] = 26
        if self.Color[0] > 230:
            self.Color[0] = 229
        if self.Color[1] > 230:
            self.Color[1] = 229
        if self.Color[2] > 230:
            self.Color[2] = 229
        self.bar = health.HealthBar(4,['b','g','y','r'])
        self.level = 1
        self.Control = controls
        self.Playerpoints = 0
        self.HP = 5
        self.TB = 10
        self.SpSet = SpSet
        self.Totalpoints = 0
        self.point = 0
        self.__invincibility = 0
        pygame.sprite.Sprite.__init__(self)
        self.prev_pos = []
        self.speed = SPEED
        self.dir = 1
        self.living = True
        #spriteList holds virtual copies of all possible images.~~~~~~~~~~~~~~~~~~
        self.spriteList = []
        self.spriteList.append(pygame.image.load("transparent.png"))
        #self.spriteList.append(pygame.image.load(self.SpSet + "R.png"))
        self.spriteList.append(pygame.transform.rotate(spr,-90))
        #self.spriteList.append(pygame.image.load(self.SpSet + "U.png"))
        self.spriteList.append(spr)
        #self.spriteList.append(pygame.image.load(self.SpSet + "L.png"))
        self.spriteList.append(pygame.transform.rotate(spr,90))
        #self.spriteList.append(pygame.image.load(self.SpSet + "D.png"))
        self.spriteList.append(pygame.transform.rotate(spr,180))
        self.spriteList.append(pygame.image.load("glow.png"))
        self.spriteList.append(pygame.image.load("glow_damage.png"))
        #end spriteList creation
        #TODO  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        self.image = self.spriteList[mod].copy()
        self.image.blit(self.spriteList[self.dir],(0,0))
        self.rect = self.image.get_rect()
        self.rect.center = (SPEED,SPEED)
        self.clockmod = 0
        self.__clockmem = 0
        self.reflect.begin()
    def begin(self, x, y, bleh):
        self.living = True
        self.prev_pos = []
        self.dir = 1
        self.rect.centerx = SPEED * 5 * self.Control
        self.rect.centery = SPEED * 5 * self.Control
        self.HP = 5
        self.bar.SetHP(self.HP)
        self.clockmod = 0
    def update(self, COL):
        self.x = SPEED * 5 * self.Control
        self.y = SPEED * 5 * self.Control
        if self.TB >= 200:
            self.TB = 0
            if self.clockmod <30:
                self.clockmod += 2
            self.level += 1
            if not lb.living:
                lb.begin(SPEED*10, SPEED*10, lb.dir)
        if self.living:
            if self.rect.centery >= screen.get_height()+ SPEED:
                self.Stop()
            if self.rect.centerx >= screen.get_width()+ SPEED:
                self.Stop()
            if self.rect.centery <= 0-SPEED:
                self.Stop()
            if self.rect.centerx <= 0-SPEED:
                self.Stop()
            
# Sets the self.dir variable. Helps ensure constant movement despite a player not pressing a button
            if self.Control == 1: #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
                if pygame.key.get_pressed()[pygame.K_RIGHT]:
                    self.dir = 1
                elif pygame.key.get_pressed()[pygame.K_DOWN]:
                    self.dir = 4
                elif pygame.key.get_pressed()[pygame.K_UP]:
                    self.dir = 2
                elif pygame.key.get_pressed()[pygame.K_LEFT]:
                    self.dir = 3
            elif self.Control ==2: #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                if pygame.key.get_pressed()[pygame.K_d]:
                    self.dir = 1
                elif pygame.key.get_pressed()[pygame.K_s]:
                    self.dir = 4
                elif pygame.key.get_pressed()[pygame.K_a]:
                    self.dir = 3
                elif pygame.key.get_pressed()[pygame.K_w]:
                    self.dir = 2
            elif self.Control == 3: #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                if pygame.key.get_pressed()[pygame.K_l]:
                    self.dir = 1
                elif pygame.key.get_pressed()[pygame.K_k]:
                    self.dir = 4
                elif pygame.key.get_pressed()[pygame.K_j]:
                    self.dir = 3
                elif pygame.key.get_pressed()[pygame.K_i]:
                    self.dir = 2
            #elif self.Control == .1:#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            if pygame.key.get_pressed()[pygame.K_F9]:
               vol = pygame.mixer.music.get_volume()
               if vol <= .03:
                  pygame.mixer.music.set_volume(0.0)
               else:
                  pygame.mixer.music.set_volume((vol-0.02))
            if pygame.key.get_pressed()[pygame.K_F10]:
               vol = pygame.mixer.music.get_volume()
               if vol >= .97:
                  pygame.mixer.music.set_volume(1.0)
               else:
                  pygame.mixer.music.set_volume((vol+0.02))
            if pygame.key.get_pressed()[pygame.K_m]:
                for Bike in Sprites:
                    if Bike.dir == 1:#right
                        Bike.refVel2 = [Bike.speed*3,0]
                    if Bike.dir == 3:#left
                        Bike.refVel2 = [-Bike.speed*3,0]
                    if Bike.dir == 2:#up
                        Bike.refVel2 = [0,-Bike.speed*3]
                    if Bike.dir == 4:#down
                        Bike.refVel2 = [0, Bike.speed*3]
                    Bike.reflect2.moveTo((Bike.rect.center[0]-Bike.refVel2[0],Bike.rect.center[1]-Bike.refVel2[1]))
                    Bike.reflect2.begin()
            if pygame.key.get_pressed()[pygame.K_SPACE]:
                for Bike in Sprites:
                    if Bike.dir == 1:#right
                        Bike.refVel = [Bike.speed*4/3,0]
                    if Bike.dir == 3:#left
                        Bike.refVel = [-Bike.speed*4/3,0]
                    if Bike.dir == 2:#up
                        Bike.refVel = [0,-Bike.speed*4/3]
                    if Bike.dir == 4:#down
                        Bike.refVel = [0, Bike.speed*4/3]
                    Bike.reflect.moveTo(Bike.rect.center)
                    Bike.reflect.begin()
                for Bike in Enemies:
                    if len(Bike.prev_pos) > 1:
                        for i in range (0,len(Bike.prev_pos) - 1):
                            Bike.prev_pos.pop(0)
                    elif len(Bike.prev_pos) > 0:
                        Bike.prev_pos.pop(0)
# Moves the Character's bike              
            if self.dir == 1:
                self.moveright()
            if self.dir == 4:
                self.movedown()
            if self.dir == 3:
                self.moveleft()
            if self.dir == 2:
                self.moveup()
            for Bike in Enemies:
               if self.rect.center in Bike.prev_pos:
                   self.Stop()
            if self.rect.center in self.prev_pos:
                self.Stop()
                self.__invincibility += int((self.clockmod/4+1)*3)
                
            self.prev_pos.append(self.rect.center)
# Actually sets the sprite, using the reference
        else:
            pygame.draw.circle(screen, ((COL/5) % 254, (COL/ 7) % 254, (COL/9) % 254), self.rect.center, 12, 1)
            self.dir = 0 #aka dead/ transparent
            if len(self.prev_pos) > 0:
                self.prev_pos.pop(0)
                
        if len(self.prev_pos) > 1:
            pygame.draw.lines(screen,(self.Color[0]-25+((COL/7)%50),self.Color[1]-25+((COL/13)%50),self.Color[2]-25+((COL/9)%50)),False,self.prev_pos, 2)
            
        if len(self.prev_pos) > (TRAIL_LIMIT + self.TB):
            self.prev_pos.pop(0)
            if len(self.prev_pos) > (TRAIL_LIMIT + self.TB):
                self.prev_pos.pop(0)
        if self.__invincibility > 0:
            self.__invincibility -= self.clockmod + 1
            self.image = self.spriteList[mod + 1].copy()
        else:
            self.image = self.spriteList[mod].copy()
        self.image.blit(self.spriteList[self.dir],(0,0))
        self.bar.SetHP(self.HP/10)
        self.bar.Draw(self.image,(0, self.image.get_height()-6))
        if not self.reflect.stopped:
            self.reflect.move(self.refVel)
            #self.reflect.move([18,0])
            self.reflect.update()
            if self.reflect.pos[0]<0 or self.reflect.pos[0] > screen.get_width() or self.reflect.pos[1]<0 or self.reflect.pos[1] > screen.get_height():
                self.reflect.end() 
                
        if not self.reflect2.stopped:
            self.reflect2.move(self.refVel2)
            #self.reflect.move([18,0])

            self.reflect2.update()
            if self.reflect2.pos[0]<0 or self.reflect2.pos[0] > screen.get_width() or self.reflect2.pos[1]<0 or self.reflect2.pos[1] > screen.get_height():
                self.reflect2.end() 

    def Stop(self):
        if self.__invincibility <= 0:
                self.HP-= 1
                self.__invincibility = 15 + int(self.clockmod/2)
        if not self.HP:
            self.living = False
            self.reflect.end()
    def movedown(self):
        self.rect.centery += self.speed
    def moveup(self):
        self.rect.centery -= self.speed
    def moveleft(self):
        self.rect.centerx -= self.speed
    def moveright(self):
        self.rect.centerx += self.speed
    def addpoints(self):
        self.point += 10
        

class LightBike4(pygame.sprite.Sprite):
    def __init__(self, direction, R = 25, G = 0, B = 0, spriteloc = "gliders/DO" ):
        pygame.sprite.Sprite.__init__(self)
        
        #this is LB shit
        self.Totalpoints = 0
        self.Playerpoints = 0
        self.HP = 10
        self.TB = 10
        self.level = 1
        #end of lb shit
        if R < 25:
            R = 25
        if G < 25:
            G = 25
        if B < 25:
            B = 25
        if R > 230:
            R = 230
        if G > 230:
            G = 230
        if B > 230:
            B = 230
        self.R = R
        self.G = G
        self.B = B
        self.IQ = 10
        self.SHP = 2
        self.prev_pos = []
        self.speed = SPEED
        self.dir = direction
        self.living = True
        self.hasdirectionchanged = False
        self.Drect = Rect(0,0,8, 28)
        self.Urect = Rect(0,0,8, 28)
        self.Lrect = Rect(0,0,28, 8)
        self.Rrect = Rect(0,0,28, 8)
        self.Lclose = Rect(0,0, 14, 14)
        self.Dclose = Rect(0,0, 14, 14)
        self.Uclose = Rect(0,0, 14, 14)
        self.Rclose = Rect(0,0, 14, 14)
        self.Rfar = 0
        self.Lfar = 0
        self.Ufar = 0
        self.Dfar = 0
        self.Rnear = 0
        self.Lnear = 0
        self.Unear = 0
        self.Dnear = 0
        self.spriteList = []
        self.spriteList.append(pygame.image.load('transparent.png'))
        self.spriteList.append(pygame.image.load(spriteloc + "R.png"))
        self.spriteList.append(pygame.image.load(spriteloc + "U.png"))
        self.spriteList.append(pygame.image.load(spriteloc + "L.png"))
        self.spriteList.append(pygame.image.load(spriteloc + "D.png"))
        self.thisimage = spriteloc
        #self.image = pygame.image.load("glow.png")
        self.image = self.spriteList[self.dir]
        self.rect = self.image.get_rect()
        self.rect.center = (SPEED * 10, SPEED * 20)
    def Stop(self):
        self.living = False
        self.begin( 6*SPEED* random.randint(3, 15), 2*SPEED* random.randint(3, 29), 3)
        lb.Playerpoints += 10 * (int(lb.clockmod/3)+1)
        lb.Totalpoints += 10 * (int(lb.clockmod/3)+1)
        lb.HP += self.SHP
        lb.TB += 10
    def begin(self, x, y, direction):
        self.living = True
        self.prev_pos = []
        self.dir = direction
        self.rect.centerx = x
        self.rect.centery = y
    def check(self):
        for Bike in Sprites:
            for point in Bike.prev_pos:
                if self.Rrect.collidepoint(point) or self.Rrect.right > WINDOWW:
                    self.Rfar = 1
                if self.Lrect.collidepoint(point) or self.Lrect.left < 0:
                    self.Lfar = 1
                if self.Urect.collidepoint(point) or self.Urect.top < 0:
                    self.Ufar = 1
                if self.Drect.collidepoint(point) or self.Drect.bottom > WINDOWH:
                    self.Dfar = 1
                if self.Dclose.collidepoint(point) or self.Dclose.bottom > WINDOWH:
                    self.Dnear = 1
                if self.Uclose.collidepoint(point) or self.Uclose.top < 0:
                    self.Unear = 1
                if self.Lclose.collidepoint(point) or self.Lclose.left < 0:
                    self.Lnear = 1
                if self.Rclose.collidepoint(point) or self.Rclose.right > WINDOWW:
                    self.Rnear = 1
        for point in self.prev_pos:
            if self.Rrect.collidepoint(point) or self.Rrect.right > WINDOWW:
                self.Rfar = 1
            if self.Lrect.collidepoint(point) or self.Lrect.left < 0:
                self.Lfar = 1
            if self.Urect.collidepoint(point) or self.Urect.top < 0:
                self.Ufar = 1
            if self.Drect.collidepoint(point) or self.Drect.bottom > WINDOWH:
                self.Dfar = 1
            if self.Dclose.collidepoint(point) or self.Dclose.bottom > WINDOWH:
                self.Dnear = 1
            if self.Uclose.collidepoint(point) or self.Uclose.top < 0:
                self.Unear = 1
            if self.Lclose.collidepoint(point) or self.Lclose.left < 0:
                self.Lnear = 1
            if self.Rclose.collidepoint(point) or self.Rclose.right > WINDOWW:
                self.Rnear = 1
    def collisionIQ(self):
        if self.dir == 3:
            #left
            if self.IQ >= 2:
                if self.Lnear:
                    if self.Dnear and self.Unear:
                        #Action(attack, reverse, jump, etc)
                        self.hasdirectionchanged = True
                    elif self.Unear:
                        self.dir = 4
                        self.hasdirectionchanged = True
                    elif self.Dnear:
                        self.dir = 2
                        self.hasdirectionchanged = True
                    elif self.Dfar and self.Ufar:
                        if random.randint(0,3) > 1:
                            self.dir = 2
                        else:
                            self.dir = 4
                        self.hasdirectionchanged = True
                    else:
                        if self.rect.centery <= WINDOWH/2:
                            self.dir = 4
                        else:
                            self.dir = 2
                        self.hasdirectionchanged = True
                elif self.Lfar:
                    if self.Dnear and self.Unear:
                        #Action(attack, reverse, jump, etc)
                        self.hasdirectionchanged = True
                    elif self.Unear:
                        self.dir = 4
                        self.hasdirectionchanged = True
                    elif self.Dnear:
                        self.dir = 2
                        self.hasdirectionchanged = True
                    elif self.Dfar:
                        self.dir = 2
                        self.hasdirectionchanged = True
                    elif self.Ufar:
                        self.dir = 4
                        self.hasdirectionchanged = True
            else:
                if self.Lfar:
                    if self.Dfar and self.Ufar:
                        if self.rect.centery <= WINDOWH/2:
                            self.dir = 4
                        else:
                            self.dir = 2
                        self.hasdirectionchanged = True
                    elif self.Ufar:
                        self.dir = 4
                        self.hasdirectionchanged = True
                    elif self.Dfar:
                        self.dir = 2
                        self.hasdirectionchanged = True
                    else:
                        if self.rect.centery <= WINDOWH/2:
                            self.dir = 4
                            self.hasdirectionchanged = True
                        else:
                            self.dir = 2
                            self.hasdirectionchanged = True
        elif self.dir == 1:
            if self.IQ >= 2:
                if self.Rnear:
                    if self.Unear and self.Dnear:
                        #Action(attack, reverse, jump, etc)
                        self.hasdirectionchanged = True
                    elif self.Unear:
                        self.dir = 4
                        self.hasdirectionchanged = True
                    elif self.Dnear:
                        self.dir = 2
                        self.hasdirectionchanged = True
                    elif self.Dfar and self.Ufar:
                        if self.rect.centery <= WINDOWH/2:
                            self.dir = 4
                        else:
                            self.dir = 2
                        self.hasdirectionchanged = True
                    else:
                        if self.rect.centery <= WINDOWH/2:
                            self.dir = 4
                        else:
                            self.dir = 2
                        self.hasdirectionchanged = True
                elif self.Rfar:
                    if self.Dnear and self.Unear:
                        #Action(attack, reverse, jump, etc)
                        self.hasdirectionchanged = True
                    elif self.Unear:
                        self.dir = 4
                        self.hasdirectionchanged = True
                    elif self.Dnear:
                        self.dir = 2
                        self.hasdirectionchanged = True
                    elif self.Dfar:
                        self.dir = 2
                        self.hasdirectionchanged = True
                    elif self.Ufar:
                        self.dir = 4
                        self.hasdirectionchanged = True
            else:
                if self.Rfar:
                    if self.Dfar and self.Ufar:
                        if random.randint(0,3) > 1:
                            self.dir = 4
                        else:
                            self.dir = 2
                        self.hasdirectionchanged = True
                    elif self.Ufar:
                        self.dir = 4
                        self.hasdirectionchanged = True
                    elif self.Dfar:
                        self.dir = 2
                        self.hasdirectionchanged = True
                    else:
                        if self.rect.centery <= WINDOWH/2:
                            self.dir = 4
                            self.hasdirectionchanged = True
                        else:
                            self.dir = 2
                            self.hasdirectionchanged = True
        elif self.dir == 2:
            if self.IQ >= 2:
                if self.Unear:
                    if self.Lnear and self.Rnear:
                        #Action(attack, reverse, jump, etc)
                        self.hasdirectionchanged = True
                    elif self.Lnear:
                        self.dir = 1
                        self.hasdirectionchanged = True
                    elif self.Rnear:
                        self.dir = 3
                        self.hasdirectionchanged = True
                    elif self.Rfar and self.Lfar:
                        if random.randint(0,2) > 1:
                            self.dir = 3
                        else:
                            self.dir = 1
                        self.hasdirectionchanged = True
                    else:
                        if self.rect.centerx <= WINDOWW/2:
                            self.dir = 1
                        else:
                            self.dir = 3
                        self.hasdirectionchanged = True
                elif self.Ufar:
                    if self.Lnear and self.Rnear:
                        #Action(attack, reverse, jump, etc)
                        self.hasdirectionchanged = True
                    elif self.Rnear:
                        self.dir = 3
                        self.hasdirectionchanged = True
                    elif self.Lnear:
                        self.dir = 1
                        self.hasdirectionchanged = True
                    elif self.Rfar:
                        self.dir = 3
                        self.hasdirectionchanged = True
                    elif self.Lfar:
                        self.dir = 1
                        self.hasdirectionchanged = True
            else:
                if self.Ufar:
                    if self.Lfar and self.Rfar:
                        if random.randint(0,3) > 1:
                            self.dir = 1
                        else:
                            self.dir = 3
                        self.hasdirectionchanged = True
                    elif self.Lfar:
                        self.dir = 1
                        self.hasdirectionchanged = True
                    elif self.Rfar:
                        self.dir = 3
                        self.hasdirectionchanged = True
                    else:
                        if self.rect.centerx <= WINDOWW/2:
                            self.dir = 1
                            self.hasdirectionchanged = True
                        else:
                            self.dir = 3
                            self.hasdirectionchanged = True
        
        elif self.dir == 4:
            if self.IQ >= 2:
                if self.Dnear:
                    if self.Rnear and self.Lnear:
                        #Action(attack, reverse, jump, etc)
                        self.hasdirectionchanged = True
                    elif self.Lnear:
                        self.dir = 1
                        self.hasdirectionchanged = True
                    elif self.Rnear:
                        self.dir = 3
                        self.hasdirectionchanged = True
                    elif self.Lfar and self.Rfar:
                        if random.randint(0,3) > 1:
                            self.dir = 1
                        else:
                            self.dir = 3
                        self.hasdirectionchanged = True
                    else:
                        if self.rect.centerx <= WINDOWW/2:
                            self.dir = 1
                        else:
                            self.dir = 3
                        self.hasdirectionchanged = True
                elif self.Dfar:
                    if self.Rnear and self.Lnear:
                        #Action(attack, reverse, jump, etc)
                        self.hasdirectionchanged = True
                    elif self.Lnear:
                        self.dir = 1
                        self.hasdirectionchanged = True
                    elif self.Rnear:
                        self.dir = 3
                        self.hasdirectionchanged = True
                    elif self.Rfar:
                        self.dir = 3
                        self.hasdirectionchanged = True
                    elif self.Lfar:
                        self.dir = 1
                        self.hasdirectionchanged = True
            else:
                if self.Dfar:
                    if self.Rfar and self.Lfar:
                        if random.randint(0,3) > 1:
                            self.dir = 3
                        else:
                            self.dir = 1
                        self.hasdirectionchanged = True
                    elif self.Lfar:
                        self.dir = 1
                        self.hasdirectionchanged = True
                    elif self.Rfar:
                        self.dir = 3
                        self.hasdirectionchanged = True
                    else:
                        if self.rect.centerx <= WINDOWW/2:
                            self.dir = 1
                            self.hasdirectionchanged = True
                        else:
                            self.dir = 3
                            self.hasdirectionchanged = True
        if not self.hasdirectionchanged:
            if random.randint(0, 100) >= 90:
                if self.Dfar:
                    self.dir = 2
                    self.hasdirectionchanged = True
                if self.Ufar:
                    self.dir = 4
                    self.hasdirectionchanged = True
                if self.Lfar:
                    self.dir = 1
                    self.hasdirectionchanged = True
                if self.Rfar:
                    self.dir = 3
                    self.hasdirectionchanged = True
    def checkavoidance(self):
        if random.randint(0, 10) >= 9.6:
            if self.dir == 3:
                if self.IQ > 7:
                    None#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~RETURN HERE TO FINISH 
            elif self.dir == 4:
                for Bike in Sprites:
                    for point in Bike.prev_pos:
                        if self.Rrect.collidepoint(point) or self.Rrect.right > WINDOWW:
                            if self.Lrect.collidepoint(point):
                                self.dir = 4
                                return True
                            else:
                                self.dir = 3
                                return True
                        elif self.Lrect.collidepoint(point) or self.Lrect.left< 0:
                            if self.Rrect.collidepoint(point):
                                self.dir = 4
                                return True
                            else:
                                self.dir = 1
                                return True
                for point in self.prev_pos:
                    if self.Rrect.collidepoint(point) or self.Rrect.right > WINDOWW:
                        if self.Lrect.collidepoint(point):
                            self.dir = 4
                            return True
                        else:
                            self.dir = 3
                            return True
                    elif self.Lrect.collidepoint(point) or self.Lrect.left< 0:
                        if self.Rrect.collidepoint(point):
                            self.dir = 4
                            return True
                        else:
                            self.dir = 1
                            return True
            elif self.dir == 1:
                for Bike in Sprites:
                    for point in Bike.prev_pos:
                        if self.Urect.collidepoint(point) or self.Urect.top <0:
                            if self.Drect.collidepoint(point):
                                self.dir = 1
                                return True
                            else:
                                self.dir = 4
                                return True
                        elif self.Drect.collidepoint(point) or self.Drect.bottom >  WINDOWH:
                            if self.Urect.collidepoint(point):
                                self.dir = 1
                                return True
                            else:
                                self.dir = 2
                                return True
                for point in Bike.prev_pos:
                    if self.Urect.collidepoint(point) or self.Urect.top <0:
                        if self.Drect.collidepoint(point):
                            self.dir = 1
                            return True
                        else:
                            self.dir = 4
                            return True
                    elif self.Drect.collidepoint(point) or self.Drect.bottom >  WINDOWH:
                        if self.Urect.collidepoint(point):
                            self.dir = 1
                            return True
                        else:
                            self.dir = 2
                            return True
            elif self.dir == 2:
                for Bike in Sprites:
                    for point in Bike.prev_pos:
                        if self.Rrect.collidepoint(point) or self.Rrect.right > WINDOWW:
                            if self.Lrect.collidepoint(point):
                                self.dir = 2
                                return True
                            else:
                                self.dir = 3
                                return True
                        elif self.Lrect.collidepoint(point) or self.Lrect.left< 0:
                            if self.Rrect.collidepoint(point):
                                self.dir = 2
                                return True
                            else:
                                self.dir = 1
                                return True
                for point in Bike.prev_pos:
                    if self.Rrect.collidepoint(point) or self.Rrect.right > WINDOWW:
                        if self.Lrect.collidepoint(point):
                            self.dir = 2
                            return True
                        else:
                            self.dir = 3
                            return True
                    elif self.Lrect.collidepoint(point) or self.Lrect.left< 0:
                        if self.Rrect.collidepoint(point):
                            self.dir = 2
                            return True
                        else:
                            self.dir = 1
                            return True
        else:
            return False
    def update(self, COL):
        self.x = WINDOWW - 3 * SPEED
        self.y = WINDOWH - 3 * SPEED
        self.Drect.centerx = self.rect.centerx
        self.Drect.centery = self.rect.centery + 24
        self.Rrect.centerx = self.rect.centerx + 24
        self.Rrect.centery = self.rect.centery
        self.Lrect.centerx = self.rect.centerx - 24
        self.Lrect.centery = self.rect.centery
        self.Urect.centerx = self.rect.centerx
        self.Urect.centery = self.rect.centery - 24
        self.Uclose.centerx = self.rect.centerx
        self.Uclose.centery = self.rect.centery - 12
        self.Lclose.centerx = self.rect.centerx - 12
        self.Lclose.centery = self.rect.centery 
        self.Dclose.centerx = self.rect.centerx
        self.Dclose.centery = self.rect.centery + 12
        self.Rclose.centerx = self.rect.centerx +12
        self.Rclose.centery = self.rect.centery 
        if self.living:
            self.check()
            if random.randint(0, 10) <= self.IQ:
                self.collisionIQ()
            #if not self.hasdirectionchanged:
              #  self.checkavoidance()
            self.hasdirectionchanged = False
            self.Rfar = 0
            self.Rnear = 0
            self.Lfar = 0
            self.Lnear = 0
            self.Ufar = 0
            self.Unear = 0
            self.Dfar = 0
            self.Dnear = 0
            
# Sets the self.dir variable. Helps ensure constant movement despite a player not pressing a button
# Moves the Character's bike              
            if self.dir == 1:
                self.moveright()
            if self.dir == 4:
                self.movedown()
            if self.dir == 3:
                self.moveleft()
            if self.dir == 2:
                self.moveup()
            for Bike in Sprites:
               if (self.rect.center in Bike.prev_pos):
                   self.Stop()
            if (self.rect.center in self.prev_pos):
                self.Stop()
            self.prev_pos.append(self.rect.center)
# Actually sets the sprite, using the reference
        else:
            pygame.draw.circle(screen, (255, 20, 20), self.rect.center, 12, 1)
 
         
            self.dir = 0
            if len(self.prev_pos) > 0:
                self.prev_pos.pop(0)
        if len(self.prev_pos) > 1:
            pygame.draw.aalines(screen,(self.R-25+((COL/7)%50),self.G-25+((COL/13)%50),self.B-25+((COL/9)%50)),False,self.prev_pos)
        if len(self.prev_pos) > (TRAIL_LIMIT + lb.TB):
            self.prev_pos.pop(0)
            if len(self.prev_pos) > (TRAIL_LIMIT + lb.TB):
                self.prev_pos.pop(0)
        
        self.image = self.spriteList[self.dir]
    def movedown(self):
        self.rect.centery += self.speed
    def moveup(self):
        self.rect.centery -= self.speed
    def moveleft(self):
        self.rect.centerx -= self.speed
    def moveright(self):
        self.rect.centerx += self.speed
    def addpoints(self):
        self.point += 10





clock = pygame.time.Clock()

lb = LightBike( 1, "lights/GLOWGREEN")
lb3 = []
lb3.append(LightBike4(3, 240, 60, 50))
lb3.append(LightBike4(1, 240, 60, 50))
lb3.append(LightBike4(2, 240, 60, 50))
lb3.append(LightBike4(4, 240, 60, 50))
lb3.append(LightBike4(3, 240, 60, 50))
lb3.append(LightBike4(1, 240, 60, 50))
lb3.append(LightBike4(2, 240, 60, 50))
lb3.append(LightBike4(4, 240, 60, 50))


bgi = GenerateBGI.GenerateTron((WINDOWW,WINDOWH), 3*SPEED, (20,20,40))
Sprites = pygame.sprite.Group(lb)
Enemies = pygame.sprite.Group(lb3)
Corners = []
Corners.append((1,1))
Corners.append((WINDOWW- 2, 1))
Corners.append((WINDOWW-2, WINDOWH-2))
Corners.append((1, WINDOWH-2))
Corners.append((1,1))
Stats = pygame.font.Font("fonts/B.ttf", int(Winw * 4))
Title = pygame.font.Font("fonts/T.ttf", int(Winw * 8)) 
Credit = pygame.font.Font("fonts/F.ttf", int(Winw * 4))
Timer = pygame.font.Font("fonts/T.ttf", int(Winw * SPEED/1.5))
pygame.mixer.init()
Play = True
def main():
    CLOCKRATE = 30
    Playerpoints = 0
    Totalpoints = 0
    pygame.mixer.music.load( 'sounds/gamemusic.mp3')
    pygame.mixer.music.play(-1)
    play = True
    xs = 1
    PAUSE = 5
    P = 0
    while play:
        CLOCKRATE = 30
        #if pygame.time.get_ticks() % 541 >= 400:
        #    lb3.append(LightBike4(2))
        COL = pygame.time.get_ticks()
        #CRED = Title.render( ("Pts/Tot: " + str(lb.Playerpoints) + "/" + str(lb.Totalpoints)) , False, ((COL/5) % 254, (COL/ 7) % 254, (COL/9) % 254))
        TITLE = Title.render( ("HP: "+ str(lb.HP) + "\tPTS: " + str(lb.Playerpoints)), False, ((COL/5) % 50+60, (COL/ 7) % 50+60, (COL/9) % 50+60))
        STATS = Title.render( "Re:Tron", False, (100,100,100))
        #LVL = Title.render( ("Level: " + str(lb.level)), False, ((COL/5) % 254, (COL/ 7) % 254, (COL/9) % 254))
        pygame.event.set_blocked((pygame.MOUSEMOTION, pygame.MOUSEBUTTONUP, pygame.MOUSEBUTTONDOWN))
        if pygame.key.get_pressed()[pygame.K_ESCAPE]:
            play = False
            Play = False
        if lb.living == False:
            from datetime import datetime
            with open('HIGHSCORES.txt', 'a') as fload:
                fload.write('\n' + "Score: " + str(lb.Playerpoints) + "\t\t\t Date: " + str(datetime.now()))
                fload.close()
            for b in range(len(lb3)): 
                lb3[b].begin(((543*b) % (WINDOWW/SPEED - 1) + 1) * SPEED , ((554*b)% (WINDOWH/SPEED - 1) + 1) * SPEED, lb3[b].dir)
            PAUSE = 5
            for Bike in Sprites:
                lb.Playerpoints= 0
                lb.TB = 0
                lb.level = 1
                Bike.begin(SPEED*10, SPEED*10, lb.dir)
        if pygame.key.get_pressed()[K_2]:
            CLOCKRATE = 15
        elif pygame.key.get_pressed()[K_3]:
            CLOCKRATE = 10
        elif pygame.key.get_pressed()[K_4]:
            CLOCKRATE = 7
        elif pygame.key.get_pressed()[K_1]:
            CLOCKRATE = 3
        if pygame.key.get_pressed()[K_p]:
            P = 1
				    #pygame.mixer.music.pause()
        while P == 1:
            pygame.event.get()
            pygame.mixer.music.pause()
            pygame.time.wait(250)
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
								    #pygame.mixer.music.unpause()
										P = 0
										pygame.mixer.music.unpause()
        clock.tick(CLOCKRATE + lb.clockmod)
        
        pygame.mouse.set_visible(False)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                play = False
       
        #bgi = pygame.image.load('BlackBackground.gif5')
        screen.blit(bgi,(0,0))
        #screen.fill(((lb.TB*1.2),(lb.TB*1.2),(lb.TB*1.2)))
        #screen.fill(((COL/50) % 20,(COL/70)% 20,(COL/10)%255))
        pygame.draw.lines(screen,((COL/5) % 254, (COL/ 7) % 254, (COL/9) % 254),False, Corners, int((Winw + Winh) / 3))
        
        #Sprites.update()
        screen.blit(TITLE, ( WINDOWW/2 - TITLE.get_width() /2 , WINDOWH/2 -TITLE.get_height()))
        screen.blit(STATS, (WINDOWW/2 - STATS.get_width()/2, WINDOWH/3 ))
        #screen.blit(CRED, (WINDOWW/2 - CRED.get_width()/2, WINDOWH/2 + STATS.get_height()))
        #screen.blit(LVL, (WINDOWW/2 - LVL.get_width()/2, LVL.get_height()))
        Sprites.update(pygame.time.get_ticks())
        Enemies.update(pygame.time.get_ticks())
        Sprites.draw(screen)
        Enemies.draw(screen)
        while PAUSE > 0:
            screen.fill(((PAUSE-1)*15 ,(PAUSE-1)*15,(PAUSE-1) * 15))
            Sprites.update(pygame.time.get_ticks())
            Enemies.update(pygame.time.get_ticks())
            Sprites.draw(screen)
            Enemies.draw(screen)
            
            intro = Timer.render((str(PAUSE) + " Use arrow keys to move " + str(PAUSE)), False, (0,100,200))
            screen.blit(intro, ( WINDOWW/2 - intro.get_width() /2 , WINDOWH/2 - intro.get_height()))
            pygame.display.flip()
            PAUSE -= 1
            pygame.time.wait(750)
            pygame.display.flip()
        pygame.display.flip()
    pygame.mouse.set_visible(True)
    

if __name__ == "__main__":
    main()
pygame.quit()
sys.exit()
        
